...
export async function buildMsgLeaderboard(
    guildId: string,
    guildName: string,
    iconURL: string | null,
    page: number,
    ...
    const rows = await getMessageLeaderboard(guildId, timeframe, PAGE_SIZE + 1, page * PAGE_SIZE);
    ...
    if (hasPrev || hasNext) {
        card.actionRow(
            btn(`msglb:${guildId}:${page - 1}:${timeframe}`, '◀ Previous', ButtonStyle.Secondary, !hasPrev),
            btn(`msglb:${guildId}:${page + 1}:${timeframe}`, 'Next ▶', ButtonStyle.Secondary, !hasNext),
        );
    }
...
export default {
    ...
    async execute(interaction, client) {
        ...
        const card = await buildMsgLeaderboard(guild.id, guild.name, iconURL, page, timeframe, guild);
        if (!card) {
            const empty = new FadeContainer(Colours.FADE).text(`${e('stats')} No message data recorded yet.`).build();
    ...
    async prefixExecute(message, args, client) {
        ...
        const card = await buildMsgLeaderboard(guild.id, guild.name, iconURL, page, timeframe, guild);
        if (!card) {
            await message.reply(`${e('stats')} No message data recorded yet.`);
            return;
        }
    ...
    example: "/msglb timeframe:Weekly page:1",
} satisfies Command;
